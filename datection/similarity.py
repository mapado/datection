...
def discretise_schedule(schedule, grain_level="day", grain_quantity=1, forced_lower_bound=None, forced_upper_bound=None):
    """Discretise the schedule in chunks of 30 minutes"""
    sc_set = set()
    for duration_rrule in schedule:
        drr = DurationRRule(duration_rrule, forced_lower_bound=forced_lower_bound, forced_upper_bound=forced_upper_bound)
        for timepoint in drr:
            if grain_level == "min":
                ...
                timepoint = timepoint.replace(month=1,day=1,hour=0,minute=0, second=0)
                sc_set.add(timepoint)
    return sc_set


def similarity(schedule1, schedule2, grain_level="day", grain_quantity=1, forced_lower_bound=None, forced_upper_bound=None):
    """Returns the jaccard similarity distance bewteen the schedules"""
    discrete_schedule1 = discretise_schedule(schedule1, grain_level=grain_level, grain_quantity=grain_quantity, forced_lower_bound=forced_lower_bound, forced_upper_bound=forced_upper_bound)
    discrete_schedule2 = discretise_schedule(schedule2, grain_level=grain_level, grain_quantity=grain_quantity, forced_lower_bound=forced_lower_bound, forced_upper_bound=forced_upper_bound)
