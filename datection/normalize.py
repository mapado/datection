...
class Timepoint(object):
    ...
    def __sub__(self, other):
        return TimepointExclusion(self, other)
...
class TimepointExclusion(object):

    """Performs an exclusion bewteen two normalized timepoints.

    The dates generated by the excluded timepoint will not be generated
    by the first one.
    ...
    def to_db(self):
        """Add an 'exclusion' RRule to the self.timepoint.to_db() export

        """
        out = self.timepoint.to_db()
        out['exclusion'] = self.excluded.rrulestr
        return out
    ...
    def future(self, reference=date.today()):
        """A TimepointExclusion is future if the constrictive timepoint is."""
